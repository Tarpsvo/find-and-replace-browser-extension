  ...
  render() {
    const renderTab = () => {
      ...
      switch (this.state.activeTab) {
        ...
        case this.TABS.templates:
          return (
            <TemplatesPanel onTemplateSelected={this.onTemplateSelected} />
          );
        case this.TABS.help:
          return "TODO";
      }
    ...
    return (
      <div className="right-panel">
        <div
          className={
            "right-panel-content" + (this.state.expanded ? " active" : "")
          }
        >
          {renderTab()}
        </div>
        <div className="right-panel-buttons" onClick={this.closePanels}>
          {Object.keys(this.buttonNames).map((id) => (
            <div
              ...
              className={
                "menu-item" +
                (this.buttonNames[id] == this.state.activeTab
                  ? " menu-item-active"
                  : "")
              }
              title={
                this.buttonNames[id][0].toUpperCase() +
                this.buttonNames[id].slice(1)
              }
              onClick={(e) => {
                e.stopPropagation();
