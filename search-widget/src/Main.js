...
class Main extends React.Component {
  constructor(props) {
    ...
    this.handleFindInputKeyboardPress =
      this.handleFindInputKeyboardPress.bind(this);
    this.handleReplaceInputKeyboardPress =
      this.handleReplaceInputKeyboardPress.bind(this);
    ...
    this.onFavouriteSelectedInPanel =
      this.onFavouriteSelectedInPanel.bind(this);
    ...
    this.handleContentScriptApiResponse =
      this.handleContentScriptApiResponse.bind(this);
  ...
  checkIfStateInFavourites() {
    ...
    Storage.isAddedToFavourites(this.getSearchStateForFavourites()).then(
      (isAdded) => {
  ...
  toggleAdvancedSearch(e) {
    ...
    Analytics.sendEvent(
      "advanced-search",
      `advanced-search-${expanded ? "enabled" : "disabled"}`
    );
  }
  ...
  toggleAddToFavourites() {
    ...
    Storage.setInFavourites(
      this.getSearchStateForFavourites(),
      /* delete? */ alreadyAdded
    );
    this.setState({
    ...
    Analytics.sendEvent(
      "favourites",
      `favourite-${alreadyAdded ? "removed" : "added"}`
    );
  }
  ...
  renderIndividualCheckboxes() {
    const invalidSelectionInput =
      this.state.limitToSelectionInput &&
    ...
    const checkboxes = {
      ...
      LimitToSelectionCheckbox: {
        ...
        tooltip: invalidSelectionInput
          ? "You must select editable text in the page first."
          : "",
  ...
  renderTextFieldInputs() {
    ...
    const ReplaceFieldInput = (
      <input
        ...
        spellCheck="false"
        className={
          "text-input" + (this.state.useRegexInput ? " code-font" : "")
        }
  ...
  renderSearchStatus() {
    ...
    const SearchStatus = (
      <div
        className={
          "search-status-text" +
          (noSearchTarget || noOccurrences ? " status-error" : "")
        }
      >
        {noSearchTarget
          ? "No Target"
          ...
          ? "No Results"
          : `${
              this.state.contentScriptSearch.searchIndex + 1
            } of ${occurrenceCount}`}
      </div>
    );
    return SearchStatus;
  ...
  render() {
    ...
    const args = {
      disabled:
        !this.state.findTextInput ||
        this.state.contentScriptError.noSearchTarget ||
        this.state.contentScriptSearch.searchCount == 0,
    };
    const FindPrevButton = (
      <Button
        onClick={this.handleFindPrev}
        title={<FontAwesome name="chevron-up" />}
        {...args}
        small
      />
    );
    const FindNextButton = (
      <Button
        ...
        title={<FontAwesome name="chevron-down" />}
        {...args}
        small
      />
    );
    const ReplaceOneButton = (
      <Button onClick={this.handleReplaceOne} title="Replace" {...args} />
    );
    ...
    return (
      <div
        className={this.state.tallView ? "tall-view" : ""}
        style={{
          display: "flex",
        }}
      >
        <div className="main-panel">
          <div style={{ display: "flex", alignItems: "center" }}>
            {FindFieldInput}
            {FindPrevButton}
            {FindNextButton} {SearchStatus}
          </div>
          <div style={{ display: "flex", alignItems: "center" }}>
            {ReplaceFieldInput}
            {ReplaceOneButton} {ReplaceAllButton}
          </div>
          <div style={{ display: "flex" }}>
            ...
            >
              ...
              {this.state.advancedSearchExpanded &&
                checkboxes.LimitToSelectionCheckbox}
            ...
            >
              ...
              />
              <Star
                checked={this.state.addedToFavourites}
                onClick={this.toggleAddToFavourites}
                descrBefore="Save to Favourites"
                descrAfter="Saved"
              />
              {/* Advanced Search (right column) */}
              {this.state.advancedSearchExpanded &&
                checkboxes.IncludeOneLineFieldsCheckbox}
            </div>
          </div>
          {this.state.advancedSearchExpanded &&
