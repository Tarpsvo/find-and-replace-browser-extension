...
class Storage {
  constructor() {
    ...
    this.analyticsEnabledPromise = this.getFromStorage(
      this.analyticsEnabledKey
    );
  ...
  addToHistory(searchState) {
    ...
    this.historyPromise = this.historyPromise.then((history) => {
      ...
      history = history.slice(
        Math.max(0, history.length - this.historyMaxLength)
      );
      // Sync storage
      chrome.storage.local.set({
        [this.historyKey]: history,
  ...
  observeOnFavouritesChanged(func) {
    if (this.dummy) {
      func({
        ...
        h: {
          findTextInput: "very long text specified as find input",
          replaceTextInput: "very long replace input",
        },
  ...
  observeOnTemplatesChanged(func) {
    if (this.dummy) {
      func({
        ...
        b: { title: "signature", text: "Tarvo R" },
        c: {
          title: "abc",
          text:
            "This is a long text specified as template " +
  ...
  hashSearchState_(searchState) {
    ...
    const prefix =
      (searchState.findTextInput ? searchState.findTextInput : "") + ";";
    return (
      prefix +
      Object.keys(searchState)
        .sort()
        .map((x) => searchState[x].toString())
        .join(";")
    );
  }
