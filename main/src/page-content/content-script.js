...
(() => {
  ...
  function setOccurrenceIndex(index) {
    $(SELECTORS.currentHighlight, Context.doc).removeClass(
  ...
  function checkTemplatesInsertable() {
    ...
    const isCollapsed =
      Search.activeCursorSelection.start == Search.activeCursorSelection.end;
    return {
  ...
  function insertTemplate(templateText) {
    ...
    if (Search.activeCursorSelection.type == TYPES.textarea) {
      ...
      const replacedText =
        originalText.substr(0, Search.activeCursorSelection.start) +
        templateText +
      ...
      const selectionLength =
        Search.activeCursorSelection.end - Search.activeCursorSelection.start;
  ...
  function templateTransformTextCase(toUpperNotLower) {
    ...
    if (Search.activeCursorSelection.type == TYPES.textarea) {
      ...
      const selectedText = originalText.substring(
        Search.activeCursorSelection.start,
  ...
  function initTextareas($elements, isSingleLine, refocus) {
    const skipSetup =
      $elements.length == 1 && $elements.hasClass(CLASSES.textareaInput);
  ...
  function filterMarksInSelection(markGroups) {
    return markGroups.filter(({ term, $marks }) => {
      const containerTextOffset =
        Search.activeCursorSelection.type == TYPES.textarea
      ...
      const isIncluded =
        Search.activeCursorSelection.start <= containerTextOffset &&
  ...
  function highlightMatchesProcess(
    activeSelectionType,
    $activeElement,
    params
  ) {
  ...
  function updateSearch(params) {
    ...
    Search.activeCursorSelection = !activeSelection.selection
      ? null
      : {
    ...
    const limitToSelectionError =
      params.limitToSelection && !uncollapsedSelection;
    // Debug info
    ...
    const highlightMatchesPromise = highlightMatchesProcess(
      activeSelection.type,
      activeSelection.$element,
      params
    );

    const everythingEditableCssSelector = `${TEXTAREA_SELECTOR}, ${CONTENTEDITABLE_SELECTOR}, ${SINGLE_LINE_INPUT_SELECTOR}`;
    const noSearchTarget =
      activeSelection.type == TYPES.mix &&
      $(everythingEditableCssSelector, Context.doc).length == 0;

    return highlightMatchesPromise
      .then((groupedMarks) => {
        ...
        return {
          invalidRegex: false,
          invalidSelection: limitToSelectionError,
          noSearchTarget,
        };
      })
      .catch((e) => {
  ...
  function getActiveSelectionAndContext(documentContext, windowContext) {
    ...
    if (activeElement) {
      ...
      if (tagName == "textarea") {
        ...
        const start = Utils.getTextOffsetInContentEditable(
          range.startContainer,
          range.startOffset
        );
        const end = Utils.getTextOffsetInContentEditable(
          range.endContainer,
          range.endOffset
        );
        return {
        ...
        try {
          ...
          return getActiveSelectionAndContext(
            innerContext,
  ...
  function getReplaceText(text) {
    ...
    if (matches && matches.length > 0) {
      ...
      matches
        .slice()
        .reverse()
        .forEach((groupText, index) => {
          const groupTextToSubstitute = groupText
            ? groupText.replace(/\$/g, placeholderChar)
            : "";
          index = matches.length - 1 - index;
          text = text.replace(
            new RegExp("\\$" + index, "g"),
            groupTextToSubstitute
          );
        });
  ...
  function shutdownSingleLineInputsOnly() {
    $(SINGLE_LINE_INPUT_SELECTOR, Context.doc).highlightWithinTextarea(
      "destroy"
    );
  }
  ...
  function maybeDisallowOnThisSite() {
    ...
    if (hostname.includes("quora.com")) {
      ...
      spawnWarningMessage(
        "Please use Google Docs' own find and replace tool instead."
      );
      return true;
    }
    return false;
  ...
  function setUpApi() {
    ...
    function setUpMessageConnections() {
      ...
      if (apiUnavailable) {
        ...
        port = {
          postMessage(msgReply) {
            console.info(msgReply);
          },
        };
        return;
      }
    ...
    function handleApiCall(msg) {
      ...
      switch (msg.action) {
        ...
        case "updateSearch":
          ...
          updateSearch(msg.data).then((errors) => {
            ...
            if (!errors.invalidRegex) {
              Object.assign(
                response.data,
                getApiResponseData(msg.action, msg.data.replaceText).data
              );
            }
        ...
        case "findNext":
          ...
          port.postMessage(
            getApiResponseData(msg.action, msg.data.replaceText)
          );
          break;
        case "findPrev":
          setOccurrenceIndex(Search.activeTermIndex - 1);
        ...
        case "replaceAll":
          ...
          break;
        case "insertTemplate":
          const { noCursorPosition, noCursorRange } =
            checkTemplatesInsertable();
          let originalText = null;
          if (!noCursorRange && msg.data.lowerCaseTransform) {
            originalText = templateTransformTextCase(
              /* toUpperNotLower */ false
            );
          } else if (!noCursorRange && msg.data.upperCaseTransform) {
            originalText = templateTransformTextCase(
              /* toUpperNotLower */ true
            );
  ...
  const Utils = {
    ...
    constructRegExpFromSearchParams(params) {
      ...
      let regexQuery = params.useRegex
        ? params.query
        : Utils.escapeRegExp(params.query);
    ...
    scrollInViewIfNotAlready($element, contextWindow) {
      ...
      if (currentTop > screenBottom || currentBottom < screenTop) {
        contextWindow.scrollTo(
          ...
          Math.max(
            0,
            Math.round((currentBottom + currentTop - $window.height()) / 2)
          )
        );
      }
    },
